...
export async function GET(req: NextRequest) {
  ...
  const users = await prisma.user.findMany({
    where: {
      ...
      status: 'ACTIVE',
      pin: { not: null },
      customRole: { name: { contains: 'caixa', mode: 'insensitive' } },
    },
...
export async function POST(req: NextRequest) {
  ...
  if (!user) {
    console.error('[caixa/auth POST] user not found – userId:', userId, 'tenantId:', tenant.id)
  ...
  if (!valid) {
    console.error('[caixa/auth POST] wrong pin – userId:', userId)
